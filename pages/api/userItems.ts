...
import { userItems } from '../../server/LMUserCases'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseType>) {
  ...
  try {
    const items = await userItems(address, contractAddress);
    console.log(`items ${JSON.stringify(items, null, ' ')}`);
    res.status(200).json({ result: true, items: items });
  } catch (ex) {
    const exception = ex as any;
    const resultBody = exception?.error?.error?.body;
    console.error(`get user items failed: ${resultBody}`);
